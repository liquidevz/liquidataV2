...
(function ($) {
    ...
    /* Header Smart Scroll (Hide on Scroll Down, Reveal on Scroll Up)
    -------------------------------------------------------------------------*/
    var headerSticky = function () {
        ...
        $(window).on("scroll", function () {
            ...
            // Near top of page
            if (st <= 40) {
                $header.removeClass("nav-up is-fixed");
                lastScrollTop = st;
                return;
            }

            // Must scroll more than delta to trigger state change
            if (Math.abs(lastScrollTop - st) <= delta) return;

            if (st > lastScrollTop && st > 80) {
                // Scrolling down -> hide navbar
                $header.addClass("nav-up").addClass("is-fixed");
            } else {
                // Scrolling up -> show navbar
                $header.removeClass("nav-up").addClass("is-fixed");
            }
            lastScrollTop = st;
        });
    };
